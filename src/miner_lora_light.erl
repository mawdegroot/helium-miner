...
handle_packets([Packet|Tail], Gateway, RxInstantLocal_us, #state{reg_region = Region} = State) ->
    ...
    case ?route(Data) of
        ...
        {noop, non_longfi} ->
            ...
            ok;
        {Type, RoutingInfo} ->
            %% normally non-Longfi packets in light mode are discarded but this will handle
            %% when the embedded router is disabled and the miner must route the packet
            lager:info("Routing ~p", [RoutingInfo]),
